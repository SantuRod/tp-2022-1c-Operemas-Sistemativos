#include "kernel_configuracion.h"
...
void destroy_configuracion() {
	...
	free(config_guardada.ip_cpu);
	free(config_guardada.algoritmo_planificacion);
...
int cargar_archivo(char * path) {
	...
	error += set_variable_str(config, "IP_CPU", 				&config_guardada.ip_cpu);
	error += set_variable_int(config, "PUERTO_CPU_DISPATCH",	&config_guardada.puerto_cpu_dispatch);
	error += set_variable_int(config, "PUERTO_CPU_INTERRUPT",	&config_guardada.puerto_cpu_interrupt);
	error += set_variable_int(config, "PUERTO_ESCUCHA", 		&config_guardada.puerto_escucha);
	error += set_variable_str(config, "ALGORITMO_PLANIFICACION",&config_guardada.algoritmo_planificacion);
	error += set_variable_int(config, "ESTIMACION_INICIAL", 	&config_guardada.estimacion_inicial);
	error += set_variable_dbl(config, "ALFA", 					&config_guardada.alfa);
	error += set_variable_int(config, "GRADO_MULTIPROGRAMACION", &config_guardada.grado_multiprogramacion);
	error += set_variable_int(config, "TIEMPO_MAXIMO_BLOQUEADO",&config_guardada.tiempo_maximo_bloqueado);
